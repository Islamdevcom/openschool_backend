...
def use_invite_code_endpoint(
    data: CodeRequest,                                           # ждём JSON: {"code": "..."}
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    ...
    if status == "already_linked":
        # ✅ Изменено: возвращаем ошибку вместо успешного сообщения
    ...
    # на случай непредвидённого текста
...
# ✅ Добавлены debug endpoints для отладки (можно убрать после тестирования)
...
def get_my_students(
    ...
    students = (
        db.query(User)
        .join(TeacherStudentLink, User.id == TeacherStudentLink.student_id)
        .filter(TeacherStudentLink.teacher_id == current_user.id)
        .all()
    )
...
def get_my_teachers(
    ...
    teachers = (
        db.query(User)
        .join(TeacherStudentLink, User.id == TeacherStudentLink.teacher_id)
        .filter(TeacherStudentLink.student_id == current_user.id)
        .all()
    )
    ...
    return [
        {
            ...
            "email": teacher.email,
            "subject": "Не указан"  # можно добавить поле subject в модель User
        }
